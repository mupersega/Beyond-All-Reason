...
- **Don't set `data-*` attributes after document load.** RmlUi ignores them. For dynamic buttons (e.g., generated via data-for), use `onclick="widget:Method()"` which *is* parsed post-init, or model functions referenced at parse time.
